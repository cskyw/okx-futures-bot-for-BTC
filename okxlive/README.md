...
└── state/               # 数据存储（自动创建）
    ...
    └── equity_history.json # 净值曲线历史数据（每 12 小时记录一次）
```
...
- TP1 半仓止盈采用“开仓后独立挂单”策略，并针对单笔仓位订单号进行精准状态追踪，不受多笔同向持仓或滑点影响。
- 止盈止损自动联动：若仓位触发初始止损，程序会自动捕捉并撤销其残留的 TP1 限价单，防止幽灵挂单。
...
- 持仓、流水、净值文件独立存储并使用绝对路径读取，重启服务不丢数据，净值曲线以 12 小时为周期平滑记录
...
`limit_offset` 默认 `0.0002`（即 0.02%），数值越大离实际价格的偏差量会更大，挂单可能更难成交；数值过小则接近市价单，手续费更高。
...
1. **初始止损（SL）**：开仓时附带 5%（`sl_pct`）保底止损单，价格反向 5% 触发平仓。
2. **TP1 半仓止盈 + 保本**：开仓单完全成交后，程序立刻向交易所发出一笔精准涨幅 3%（`tp1_pct`）的限价条件止盈单（平半仓）。交易所执行该单后，程序本地会将剩余一半仓位的止损线移至开仓均价。
3. **TP2 动态追踪止损**：
...
本项目自带一套轻量级的网页前端（位于 `dashboard/` 目录），无需安装任何前端框架，直接通过 `api_server.py` 提供的 HTTP 接口即可在浏览器中实时查看交易状态。

### 启动看板

```bash
# 确保 live_trader.py 已在后台运行后，启动 API 服务：
...
|------|------|
| **账户总览** | 实时显示总权益、可用保证金 |
| **实时持仓** | 直连 OKX API，展示当前多/空持仓均价、浮盈亏、杠杆倍数 |
| **净值曲线** | 每 12 小时自动记录一次净值快照，以折线图展示资产长期走势；支持 ALL / 7D / 30D / 90D 视图切换 |
| **统计指标** | 胜率、总收益率（基于历史成交）以及动态年化收益率 |
| **历史平仓记录** | 按时间倒序展示每笔已平仓订单，标注方向、张数、开仓价、平仓价、杠杆收益率及平仓原因 |
| **日期过滤器** | 可自定义起止日期，筛选任意时间段内的成交统计数据 |
| **系统日志终端** | 实时滚动展示 `trader.log` 的最新 70 行日志，方便远程运维 |

> **提示**：如服务器有防火墙，需开放 `5000` 端口（TCP）。若不希望公网暴露，可通过 SSH 隧道在本地访问：
> ```bash
